...
def _obj(v: Any, name: str, ds: list[str]) -> dict:
    ...
    if not isinstance(x,dict): ds.append(f"type:{name}"); return {}
    return x

def _version(v: dict, expected: str, ds: list[str]):
    if v.get("contractVersion") != expected: ds.append("version")
...
def _no_trusted_auth(v: Any, ds: list[str], prefix: str=""):
    ...
    if isinstance(v,dict):
        for k,x in v.items():
            if k in forbidden: ds.append(f"trusted-auth:{prefix}{k}")
            _no_trusted_auth(x,ds,prefix+k+".")
    elif isinstance(v,list):
        for i,x in enumerate(v): _no_trusted_auth(x,ds,prefix+str(i)+".")
...
def validate_shared_envelope(v: Any)->Validation:
    ...
    _no_trusted_auth(v,ds);return Validation(not ds,tuple(ds))

def validate_enterprise_observation(v: Any)->Validation:
    ds=[]
    if not isinstance(v,dict): return Validation(False,("type",))
    _req(v,("contractVersion","observationId","deliveryId","observationType","enterpriseId","effectiveAt","observedAt","publishedAt","correlationId","source","payload","provenance"),ds);_version(v,"1.0.0",ds)
...
def validate_command_proposal(v: Any, now: datetime|None=None)->Validation:
    ds=[]
    if not isinstance(v,dict): return Validation(False,("type",))
    _req(v,("contractVersion","proposalId","enterpriseId","createdAt","expiresAt","basis","commandContractId","commandContractVersion","commandCode","inputSchemaVersion","proposedInput","evidence","rationale","correlationId","commandIdempotencyKey"),ds);_version(v,"1.0.0",ds)
    if v.get("proposalId")==v.get("commandIdempotencyKey"): ds.append("proposal-equals-idempotency")
    _no_trusted_auth(v,ds)
    try:
        exp=datetime.fromisoformat(str(v.get("expiresAt")).replace("Z","+00:00"));n=now or datetime.now(timezone.utc)
...
def validate_operational_change(v: Any)->Validation:
    ds=[]
    if not isinstance(v,dict): return Validation(False,("type",))
    _req(v,("contractVersion","proposalId","enterpriseId","targetDefinitionType","targetDefinitionId","baseVersion","changeIntent","operations","evidence","compatibility"),ds);_version(v,"1.0.0",ds)
    if not v.get("baseVersion"): ds.append("base-version")
    if v.get("expectedCurrentVersion") is not None and str(v.get("baseVersion"))!=str(v.get("expectedCurrentVersion")): ds.append("stale-base-version")
...
def validate_enterprise_simulation(v: Any)->Validation:
    ...
    if v.get("namespace")!="HYPOTHETICAL" or v.get("writeActual") is True: ds.append("actual-write")
    return Validation(not ds,tuple(ds))

def validate_experience_proposal(v: Any, supported: dict[str,set[str]]|None=None)->Validation:
    ds=[]
    if not isinstance(v,dict): return Validation(False,("type",))
    _req(v,("contractVersion","proposalId","producedAt","experience","context","fallback"),ds);_version(v,"1.0.0",ds)
    exp=_obj(v,"experience",ds);_req(exp,("experienceId","regions"),ds);regs=exp.get("regions")
...
def validate_action_request(v: Any)->Validation:
    ds=[]
    if not isinstance(v,dict): return Validation(False,("type",))
    _req(v,("contractVersion","actionRequestId","experienceInstanceId","experienceContractVersion","capabilityId","capabilityVersion","actionSemantic","targetRef","interactionContext","submittedValues","confirmationEvidence","actorContextRef","correlationId","occurredAt","presentedStateEtag","presentedDefinitionVersion"),ds);_version(v,"1.0.0",ds)
...
def validate_evo_action_mapping(action: dict, command: dict)->Validation:
    ds=list(validate_action_request(action).diagnostics)
